...
import { HomeIcon, NewspaperIcon, HeartIcon, PhoneIcon } from '@heroicons/vue/24/outline';
...
<template>
  <footer class="bg-gradient-to-r from-emerald-800 to-green-700">
    <div class="max-w-7xl mx-auto py-12 px-4 overflow-hidden sm:px-6 lg:px-8">
      <nav class="flex flex-wrap justify-center -mx-5 -my-2" aria-label="Footer">
        <div v-for="item in navigation.main" :key="item.name" class="px-5 py-2">
          <router-link :to="item.href" class="text-base text-gray-100 hover:text-white flex items-center">
      ...
      <div class="mt-8 flex justify-center space-x-6">
        <a v-for="item in navigation.social" :key="item.name" :href="item.href" class="text-gray-100 hover:text-white">
          <span class="sr-only">{{ item.name }}</span>
          <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true" v-html="item.icon"></svg>
        </a>
      </div>
      <p class="mt-8 text-center text-base text-gray-100">
        &copy; 2024 BisaKita. Semua hak dilindungi.
      </p>
    </div>
  </footer>
</template>